...
labels = ( 'background',
    ...
    'no overtaking', 'no overtaking', 'priority at next intersection', 'priority roard', 'give way',
    'stop', 'no traffice both ways', 'no trucks', 'no entry', 'danger', 'bend left', 'bend right',
    'bend', 'uneven road', 'slippery road', 'road narrows',
    ...
    'roundabout', 'restriction ends(other)', 'restriction ends(trucks)(other)')
...
def draw_bnbox(output, image_to_classify):
    #   a.	First fp16 value holds the number of valid detections = num_valid.
    #   b.	The next 6 values are unused.
    #   c.	The next (7 * num_valid) values contain the valid detections data
    #       Each group of 7 values will describe an object/box These 7 values in order.
...
def classify_frame(ssd_mobilenet_graph, fifo_in, fifo_out, input_image):
    # keep looping
    while True:
        ...
        if not inputQueue.empty():
            ...
            ssd_mobilenet_graph.queue_inference_with_fifo_elem(fifo_in, fifo_out, inference_image.astype(numpy.float32), input_image)
            print("LoadTensor")
            # Get the result from the NCS
            #output, userobj = ssd_mobilenet_graph.GetResult()
...
def main():
    ...
    ssd_mobilenet_graph = mvncapi('graph1')
    ...
    p = Process(target=classify_frame, args=(ssd_mobilenet_graph, inputQueue, outputQueue,))
    p.daemon = True
    p.start()
    exit_app = False
    ...
    while (True):
        for input_video_file in input_video_filename_list :
            ...
            vs = FileVideoStream(input_video_file).start()
            ...
            while(True):
                ...
                display_image = vs.read()
                
                resized_image = preprocess_image(display_image)                
                ...
                if inputQueue.empty():
                    print("inputing queue")
                    inputQueue.put(resized_image)

                # if the output queue *is not* empty, grab the detections
                if not outputQueue.empty():
                    output = outputQueue.get()
                    draw_bnbox(output, display_image)
                    printf("Found")
